...
def save_data_list_to_output(data_list: list[AddressData]):
    ...
    logger.info(f'[{new_output_dir}] Пытаюсь записать данные об адресах..')
    ...
    with open(output_json_path, 'w', encoding='utf-8') as output_json:
        ...
        json.dump(json_data_to_dump, output_json, indent=4)
    ...
    with open(output_toml_path, 'w', encoding='utf-8') as output_toml:
        ...
        toml.dump(toml_data_to_dump, output_toml)

    logger.success(f'[{new_output_dir}] Записал данные об адресах')
